// should be refactored and imported from spacing
...
const LineclampValues = [1, 2, 3, 4, 5, 6, 'none']
const Lineclamp = ['line-clamp-']

const list_8 = []
Responsive.forEach((res) => {
	Lineclamp.forEach((clamp) => {
		LineclampValues.forEach((value) => {
			list_8.push(res + clamp + value)
		})
	})
})
...
Responsive.forEach((res) => {
	Zindex.forEach((zin) => {
		ZIndexValues.forEach((value) => {
			list_9.push(res + zin + value)
		})
	})
})
...
const list_10 = []
const PositionValues = ['relative', 'absolute', 'sticky', 'fixed']

Responsive.forEach((res) => {
	PositionValues.forEach((value) => {
		list_10.push(res + value)
	})
})



const PositionMoveValues = [
	8,
	16,
	20,
	24,
...
const list_11 = []
Responsive.forEach((res) => {
	PositionMove.forEach((posmove) => {
		PositionMoveValues.forEach((value) => {
			list_11.push(res + posmove + value)
		})
	})
})
...
// merge all lists and export
export const getSafelist = () => {
	const safelist = list.concat(
		list_1,
		list_2,
		list_3,
		list_4,
