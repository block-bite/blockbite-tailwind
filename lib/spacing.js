...
export const getSpacing = (prefix = "b_") => {
  // merge object
  const grid = createGridSpacing(prefix);
  const clamp = clampSpacing(prefix);
  const nativeSpacing = nativeSpacing(prefix);
  const allSpacing = Object.assign(grid, clamp, nativeSpacing);
  return allSpacing;
};
